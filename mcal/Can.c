...
Can_UnitType CanUnit[CAN_CTRL_CONFIG_CNT] =
{
  {
    /* .state = */ CANIF_CS_UNINIT,
  },
#if CAN_CTRL_CONFIG_CNT > 1
  {
    /* .state = */ CANIF_CS_UNINIT,
  },
#endif
#if CAN_CTRL_CONFIG_CNT > 2
  {
    /* .state = */ CANIF_CS_UNINIT,
  },
#endif
#if CAN_CTRL_CONFIG_CNT > 3
  {
    /* .state = */ CANIF_CS_UNINIT,
  },
#endif
#if CAN_CTRL_CONFIG_CNT > 4
  {
    /* .state = */ CANIF_CS_UNINIT,
  },
#endif
...
void Can_InitController( uint8 controller, const Can_ControllerConfigType *config)
{
  ...
  canHw->BTR0 = (config->CanControllerPropSeg << 6) | (uint8)(clock/((uint32)config->CanControllerBaudRate*1000*tq) - 1); // Prescaler
